...
public class Ch16Controller {
	...
	public String content(Model model) {
		...
		model.addAttribute("list", accounts); // content.jsp 22번 째라인 <c:forEach var="account" items="${list}">
		return "ch16/content";
	}
	
	@RequestMapping("/transaction1")
	public String transcation1(int fromAno, int toAno, int amount, HttpSession session) {
		logger.info("실행");
		...
		if(result == TransferResult.FAIL_NOT_FOUND_ACCOUNT) {
			session.setAttribute("transferError", "게좌가 존재하지 않습니다.");
		}
		return "redirect:/ch16/content";
	}
	
	@RequestMapping("/transaction2")
	public String transcation2(int fromAno, int toAno, int amount) {
		logger.info("실행");
