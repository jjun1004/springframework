...
<div class = "card m-2">
		...
		<div class="card-body">
				<div class = "card">
						...
						<div class="card-body">
							<p>서버: 세션 객체 생성-> JSESSION 쿠키 발생 </p>
				...
				<div class = "card">
					...
					<div class="card-body">
						<c:if test="${sessionMid == null }">
			                ...
			                <div class="input-group">
			                   ...
			                   <span id="mid-error" class="text-danger"></span>
			                </div>
			                <div class="input-group">
			                   ...
			                   <span id="mpassword-error" class="text-danger"></span>
			                </div>
			             </form>
			            </c:if>
					<div class = "mt-2">
					...
					<script>
							function login() { //유효성 검사는 생략
								...
								$.ajax({
									url: "loginAjax",
									data: {mid, mpassword},// {mid:mid, mpassword:mpassword} 두개가 같을 경우 생략 가능
									method: "post"
								}).done((data) => {
										//data = {result: "success"}
										...
										if(data.result === "success") {
											//현재 페이지 전체를 다시 서버에서 받아오도록 함. 하지만 ajax의 취지와 부적합.
											window.location.reload(); //location.reload(); 
										} else if(data.result === "wrongMid") {
											midError.html("아이디가 잘못 입력");
